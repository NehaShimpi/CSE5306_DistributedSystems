...
/*This is the code for BackUp Server client handler*/
...
public class Client_Handler2 extends Server_Side_Code2 implements Runnable {
	...
	public Client_Handler2(Socket socket) {
		this.socket = socket;
	}
/*Function to poll clients*/
	...
	public void run() {
		...
		try {
			...
			Server_Side_GUI2.Conversations.append("\n Main Server failed.");
				...
				/*If incoming text has 2 appended at the beginning, Client is connected */
					...
					if (count < 3) {
						...
						if (!list.contains(str)) {
							...
							dos.writeUTF("No worries! Connected to Backup Server!");
							dos.flush();
						} else {/*If client with same name already present*/
							Server_Side_GUI2.Conversations
