...
public class Client implements Runnable {
	...
	static String serverRunning = "Main";

	/*
	 * start point of client- Establishing connection and continuous listening of
	 * server output.
	 */
	public static void main(String[] args) {
		connectToServer(Constants.MAIN_PORT);
		openClientGUI();
		startNewClientThread();
	}

	public static void connectToServer(int port) {
		try {
			client = new Socket("localhost", port);/* accepts the connection at mentioned port */
	...
	public static void startNewClientThread() {
	...
	public void run() {
		try {
			...
			while (true) {
				...
				{
					FileWriter myWriter = new FileWriter("D:/Neha_Documents/Study/Spring2021/output.txt");
					myWriter.write(line.substring(1));
					myWriter.close();

				} else {
					Client_Side_GUI.convoArea
			...
			/*This patch handles the main server failure case.*/
			/*Notification to client about mains server failure*/
			Client_Side_GUI.convoArea.append("\nOops!!! Connection to "+ serverRunning+ " Server failed!!!");
			/*Client connects to backup server*/
			if(serverRunning.equals("Main")) {
	...
	private void connectToBackupServer() {
		/*Connects to new port of backup server*/
		connectToServer(utils.Constants.BACKUP_PORT);
		
		serverRunning = "Backup";
		/*Independent threads of each client starts*/
		startNewClientThread();
		
		/*Sends user connected message to backup server*/
		Client_Side_GUI.sendUserConnectedMessage();
	}
	...
	/*
	 * https://stackoverflow.com/questions/19152256/how-to-make-gui-client-to-upload
	 * -file-in-java
	 */
